...
| Cross-service Join | 클라이언트가 여러 번 호출 | 단일 쿼리로 자동 조합 |
| N+1 문제 | batch endpoint로 수동 해결 | DataLoader로 자동 해결 |
...
| 최적화 | DataLoader (자동 배치) | Batch endpoint (수동) |
...
│                           │                   │                           │
│  (Federation + DataLoader)│                   │  (Independent + Batch)    │
└───────────────────────────┘                   └───────────────────────────┘
        ▲                                                       ▲
        │                                                       │
        └───────────────────────────┬───────────────────────────┘
